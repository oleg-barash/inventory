...
{
    class MyAuthHandler : AuthenticationHandler<BasicAuthenticationOptions>
    {
        private readonly string _challenge;

        public MyAuthHandler(BasicAuthenticationOptions options)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            if (string.IsNullOrEmpty(authzValue) || !authzValue.StartsWith("Basic ", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (string.IsNullOrWhiteSpace(userDataCookie))
                    return null;
                try
                {
            ...
            {
                token = authzValue.Substring("Basic ".Length).Trim();
            }
